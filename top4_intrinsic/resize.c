    ...
    for (uint16_t c = 0; c < dst_c; c++){
        ...
        for (uint16_t h = 0; h < dst_height; h++){
            ...
            for (uint16_t w = dst_width; w; w -= w_vl){
                ...
                vint16m2_t y0x0_val_int16 = vrgather_vv_i16m2(src_data_y0, x0_seged, w_vl);
                vint16m2_t y0x1_val_int16 = vrgather_vv_i16m2(src_data_y0, x1_seged, w_vl);

                vint16m2_t y1x0_val_int16 = vrgather_vv_i16m2(src_data_y1, x0_seged, w_vl);
                vint16m2_t y1x1_val_int16 = vrgather_vv_i16m2(src_data_y1, x1_seged, w_vl);

                vint16m2_t sum1_y0x0 = vmul_vv_i16m2(y0x0_val_int16, w0_lambda_int16, w_vl);
                vint16m2_t sum4_y1x0 = vmul_vv_i16m2(y1x0_val_int16, w0_lambda_int16, w_vl);
                vint16m2_t sum1 = vmacc_vv_i16m2(sum1_y0x0, y0x1_val_int16, w1_lambda_int16, w_vl);
                vint16m2_t sum4 = vmacc_vv_i16m2(sum4_y1x0, y1x1_val_int16, w1_lambda_int16, w_vl);
