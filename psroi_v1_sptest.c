...
void psroi_23x40x392x300_test(int seed){
    srandom(seed);
    ...
    uint32_t out_size = pooled_h * pooled_w * pooled_c * rois_num;
    uint32_t src_size = src_height * src_width * pooled_h * pooled_w * pooled_c;
    
    uint8_t src_arr[360640] = {0};
    for (uint32_t i=0; i < src_size; i++)
    {
        src_arr[i] = ((rand() % (64)) );
    }

    uint16_t rois_arr[1200] = {0};
    for(uint32_t i=0; i < rois_num/2; i++)
    {
        rois_arr[i*8]   = (uint16_t)(1 * (1 << roi_shift));
        rois_arr[i*8+1] = (uint16_t)(1 * (1 << roi_shift));
        rois_arr[i*8+2] = (uint16_t)(11 * (1 << roi_shift));
        rois_arr[i*8+3] = (uint16_t)(11 * (1 << roi_shift));

        rois_arr[i*8+4] = (uint16_t)(2 * (1 << roi_shift));
        rois_arr[i*8+5] = (uint16_t)(2 * (1 << roi_shift));
        rois_arr[i*8+6] = (uint16_t)(16 * (1 << roi_shift));
        rois_arr[i*8+7] = (uint16_t)(16 * (1 << roi_shift));
    }

    uint16_t out_arr_golden[117600] = {0};
    uint16_t out_arr_vec[117600] = {0};
    struct psroi_param golden_param = {src_arr, rois_arr, out_arr_golden, \
    rois_num, pooled_h, pooled_w, pooled_c, src_height, src_width, sampling_ratio, roi_shift};
    struct psroi_param rvv_param = {src_arr, rois_arr, out_arr_vec, \
    rois_num, pooled_h, pooled_w, pooled_c, src_height, src_width, sampling_ratio, roi_shift};
    ...
    for (int out_idx=0; out_idx < out_size; out_idx++){
        printf("golden_param.out_arr %d = rvv_param.out_arr %d.\n", golden_param.out_arr[out_idx], rvv_param.out_arr[out_idx]);
    }
    ...
    if (pass) printf("psroi 23x40x392x300 test passed.\n");  
    else printf("psroi 23x40x392x300 test failed.\n");      
}
